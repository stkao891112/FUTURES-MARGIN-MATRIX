# 🚀 FUTURES MARGIN MATRIX 專案技術總結與架構文件

本文件詳細記錄了 **FUTURES MARGIN MATRIX（跨交易所永續合約保證金與爆倉清算計算機）** 的整體系統架構、所使用的核心技術棧、反爬蟲防護繞過方案、資料處理與前端 PWA 設計細節。
...
- **核心目標**：
  自動化爬取並整合 **7 大加密貨幣與美股衍生品交易所** 永續合約全幣種（包含加密貨幣與美股/指數標的）之維持保證金率 (MMR)、階梯檔位 (Tier Limits)、維持保證金扣減額 (Deduction) 與最大槓桿限制，並提供實時爆倉清算價比對計算機。
- **支援交易所**：
  1. **Binance (幣安)**
  2. **Bitget**
  3. **Bybit**
...
└── PROJECT_SUMMARY.md       # 本專案技術總結與架構文件
```
...
```mermaid
flowchart TD
    A[使用者點擊 更新資料來源 / API 呼叫 /api/refresh] --> B[server.py 接收請求並執行 main.py]
    B --> C[啟動 asyncio 7 大交易所並行爬蟲任務]
    ...
    C --> D4[OKX 爬蟲]
    C --> D5[MEXC 爬蟲]
    C --> D6[BingX 爬蟲 (NCSK 格式)]
    C --> D7[Pionex 爬蟲 (Chrome Channel & X 後綴)]
    
    D1 & D2 & D3 & D4 & D5 & D6 & D7 --> E[DataFrame 標準化 normalize_dataframe_columns]
    E --> F[寫入 data/all_exchanges_futures_margin.xlsx]
    F --> G[執行 export_json.py]
    G --> H[更新 data/tiers.json 與 index.html 直嵌數據]
    H --> I[前端 Progress Modal 達 100% 並即時重新計算爆倉價]
```
